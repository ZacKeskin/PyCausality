...
for LAG in LAGs:
    ...
    (TE_XY, TE_YX) = causality.nonlinear_TE(
        pdf_estimator="kernel", bins=bins, n_shuffles=N_SHUFFLES
    )
...
TE_axis.set_title(
    "TE versus Lag for Coupled Random Walks; Coupling Strength = " + str(SIMILARITY),
    fontsize=11,
)
...
TE_axis.legend(
    ["Linear TE (S1->S2)", "Linear TE (S2->S1)", "TE (S1->S2)", "TE (S2->S1)"],
    fontsize=8,
)
...
Z_axis.legend(
    ["Significance of TE (S1->S2)", "Significance of TE (S2->S1)"], fontsize=8
)
...
# plt.savefig(os.path.join(os.getcwd(), 'Examples', 'Plots', 'Detect_Lag.png'))
plt.show()
