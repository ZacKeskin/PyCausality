...
from .Test_Utils.Time_Series_Generate import coupled_random_walks
...
def calculate_TE(fixture, method):
    ...
    DF = coupled_random_walks(
        S1, S2, T, N, mu1, mu2, sigma1, sigma2, alpha, 0, lag, seed
    )
    ...
    if method == "linear":
        ...
        (p_XY, p_YX) = (
            causality.results["p_value_linear_XY"].iloc[0],
            causality.results["p_value_linear_YX"].iloc[0],
        )
        (Z_XY, Z_YX) = (
            causality.results["z_score_linear_XY"].iloc[0],
            causality.results["z_score_linear_YX"].iloc[0],
        )
    else:
        (TE_XY, TE_YX) = causality.nonlinear_TE(
            pdf_estimator=estimator,
            bins=bins,
            bandwidth=bandwidth,
            n_shuffles=n_shuffles,
        )
        (p_XY, p_YX) = (
            causality.results["p_value_XY"].iloc[0],
            causality.results[["p_value_YX"]].iloc[0],
        )
        (Z_XY, Z_YX) = (
            causality.results["z_score_XY"].values,
            causality.results[["z_score_YX"]].values,
        )

    # Evaluate the assertion contained within the test
    print("assertion", assertion)
    print(eval(assertion))  # noqa

    print("TE_XY:" + str(TE_XY), "TE_YX:" + str(TE_YX))
    print("p_XY:" + str(p_XY), "p_YX:" + str(p_YX))
    print("z_score_XY:" + str(Z_XY), "z_score_YX:" + str(Z_YX))
    assert eval(assertion)  # noqa


def test_generator():
    """Test generator."""
    # Test linear_TE()
    for _i, (_fixture_name, fixture) in enumerate(linear_tests.items()):
        # Pass each structure to test function
        yield calculate_TE, fixture, "linear"

    # Test nonlinear_TE()
    for _i, (_fixture_name, fixture) in enumerate(nonlinear_tests.items()):
        # Pass each structure to test function
